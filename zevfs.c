...
static int slot_for(const char *path,int create){uint8_t s[512];struct zevfs_meta *m=(struct zevfs_meta*)s;for(int i=0;i<ZEVFS_SLOTS;i++){if(ata_read28(META_LBA+i*SLOT_SECTORS,s))return -1;if(m->magic==ZEVFS_MAGIC&&eq(m->path,path))return i;}if(!create)return -1;for(int i=0;i<ZEVFS_SLOTS;i++){if(ata_read28(META_LBA+i*SLOT_SECTORS,s))return -1;if(m->magic==0){zero(s);m->magic=ZEVFS_MAGIC;for(int j=0;j<64;j++)m->path[j]=path[j];m->size=0;return i;}}return -1;}
int zevfs_ready(void){return ready;}
int zevfs_format(void){uint8_t s[512];if(ata_init())return -1;for(int i=0;i<ZEVFS_SLOTS*SLOT_SECTORS+1;i++){zero(s);if(ata_write28(META_LBA+i,s))return -1;}ready=1;return 0;}
